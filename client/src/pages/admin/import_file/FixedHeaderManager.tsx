...
export default function FixedHeaderManager({
  headerName,
  control,
  register,
  watch,
  ...
  const handleDelete = (index: number) => {
    if (window.confirm("Are you sure you want to delete this fixed header?")) {
      remove(index);
    }
  };
    ...
    <div className="space-y-3">
      ...
      <div className="flex gap-3">
        <input
          ...
          ref={inputRef}
          type="text"
          value={fixedHeaderInputs[headerName] || ""}
          onChange={(e) =>
        ...
        <button
          type="button"
          onClick={() => {
            addFixedHeader(headerName, fields, append);
        ...
        <button
          type="button"
          onClick={() => cancelFixedHeader(headerName)}
      ...
      {(errors as any)?.[headerName]?.fixed_header_input && (
        <p className="text-red-500 text-xs">
          {(errors as any)[headerName].fixed_header_input.message}
        </p>
      )}
