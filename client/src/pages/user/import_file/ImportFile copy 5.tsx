...
function ImportFile() {
  ...
  const [blockSpecialChars, setBlockSpecialChars] = useState<Record<string, boolean>>({});
  const [allowAlphaNumeric, setAllowAlphaNumeric] = useState<Record<string, boolean>>({});
  const [lengthType, setLengthType] = useState<Record<string, "variable" | "fixed">>({});
  ...
  const handleLengthTypeChange = (column: string, type: "variable" | "fixed") => {
    const updated = {
      ...lengthType,
      [column]: type,
    };
    ...
    columns.forEach((col) => {
      if (allowedLengthTypes.includes(columnMappings[col])) {
        if (lengthType[col] === "variable") {
          ...
          if (!fixed) {
            errors[col] = "Fixed length is required";
            return;
          }

          // Check if value is a valid number
          if (isNaN(Number(fixed))) {
            errors[col] = "Fixed length must be a valid number";
            return;
          }

          // Check if value is positive
          if (Number(fixed) < 0) {
            errors[col] = "Fixed length must be a positive number";
            return;
          }
        }
      }
    ...
    try {
      ...
      const columnConfig = columns.map((col) => {
        ...
        if (allowedLengthTypes.includes(columnMappings[col])) {
          ...
          if (lengthType[col] === "variable") {
            config.min_length = minLength[col] ? parseInt(minLength[col]) : null;
            config.max_length = maxLength[col] ? parseInt(maxLength[col]) : null;
          } else if (lengthType[col] === "fixed") {
            config.fixed_length = fixedLength[col] ? parseInt(fixedLength[col]) : null;
          }
        }
                    ...
                    <div className="flex items-center gap-4 pt-2 border-t border-gray-200 bg-blue-50 p-3 rounded">
                      ...
                      <label className="flex items-center gap-2 cursor-pointer">
                        <input
                          ...
                          onChange={(e) => handleBlockSpecialCharsChange(column, e.target.checked)}
                          className="w-4 h-4 rounded border-gray-300 cursor-pointer"
                        />
                        <span className="text-xs font-medium text-gray-700">Block Special Characters</span>
                      </label>
                      ...
                      <label className="flex items-center gap-2 cursor-pointer">
                        <input
                          ...
                          onChange={(e) => handleAllowAlphaNumericChange(column, e.target.checked)}
                          className="w-4 h-4 rounded border-gray-300 cursor-pointer"
                        />
                        <span className="text-xs font-medium text-gray-700">Allow Alpha Numeric</span>
                      </label>
                    </div>
                  )}

                  {/* Length Configuration */}
                  {["Text", "Number", "Decimal", "Email"].includes(columnMappings[column]) && (
                    <div className="pt-2 border-t border-gray-200 bg-green-50 p-3 rounded space-y-3">
                      <div className="text-xs font-semibold text-gray-700 mb-2">Length Configuration</div>
                      ...
                      <div className="flex items-center gap-6">
                        <label className="flex items-center gap-2 cursor-pointer">
                          <input
                            ...
                            onChange={(e) => handleLengthTypeChange(column, "variable")}
                            className="w-4 h-4 cursor-pointer"
                          />
                          <span className="text-xs font-medium text-gray-700">Variable Length</span>
                        </label>
                        ...
                        <label className="flex items-center gap-2 cursor-pointer">
                          <input
                            ...
                            onChange={(e) => handleLengthTypeChange(column, "fixed")}
                            className="w-4 h-4 cursor-pointer"
                          />
                          <span className="text-xs font-medium text-gray-700">Fixed Length</span>
                        </label>
                      </div>
                      ...
                      {lengthType[column] === "fixed" && (
                        <div className="flex-1 max-w-xs">
                          <label className="text-xs font-semibold text-gray-600 block mb-1">Length</label>
                          <input
                            type="number"
